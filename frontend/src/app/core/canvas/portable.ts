...
import type { PortableBoard, TldrawPage } from "../project-portable";
import {
  CARD_MAX_W,
  allIdeaCards,
  cardRef,
 ...
 * Reads every page (the portable form is deliberately page-less — cards and
 * edges from all pages are flattened into one list).
 */
export function exportBoard(editor: Editor): PortableBoard {
  const cards = allIdeaCards(editor);
  const cardIds = new Set(cards.map((c) => c.id));
  const refByShape = new Map<TLShapeId, string>();
  for (const c of cards) refByShape.set(c.id, cardRef(editor, c.id)!);
  const allShapes = editor.store.allRecords().filter((r): r is TLShape => r.typeName === "shape");
  ...
  return {
    ...
    edges: ideaEdges(editor, cardIds, allShapes)
      .map((e) => ({
...
/**
 * Full-fidelity tldraw snapshot of every page — names plus every shape (not
 * just idea cards), positions, arrows, and embedded assets. Exported alongside
 * the portable board so an import can restore the canvas exactly; the
 * ref-keyed board remains the stable, validated fallback. Content capture is
 * per-page and `getContentFromCurrentPage` really does mean the *current*
 * page, so this walks the pages (switching and restoring the current one).
 */
export async function exportTldrawPages(editor: Editor): Promise<TldrawPage[]> {
  const originalPageId = editor.getCurrentPageId();
  const pages: TldrawPage[] = [];
  for (const page of editor.getPages()) {
    const ids = Array.from(editor.getPageShapeIds(page.id));
    let content: TLContent | undefined;
    if (ids.length > 0) {
      if (editor.getCurrentPageId() !== page.id) editor.setCurrentPage(page.id);
      content = await editor.resolveAssetsInContent(editor.getContentFromCurrentPage(ids));
    }
    pages.push({ name: page.name, content });
  }
  if (editor.getCurrentPageId() !== originalPageId) editor.setCurrentPage(originalPageId);
  return pages;
}

/**
 * Restore a full-fidelity snapshot onto the (normally empty) live canvas: the
 * fresh editor's default page becomes the snapshot's first page (renamed),
 * further pages are created, and the first page is left as the current one.
 */
export function importTldrawPages(editor: Editor, pages: TldrawPage[]): void {
  if (pages.length === 0) return;
  const put = (content?: TLContent) => {
    if (content) editor.putContentOntoCurrentPage(content, { preservePosition: true, select: false });
  };
  const firstPageId = editor.getCurrentPageId();
  editor.renamePage(firstPageId, pages[0].name);
  put(pages[0].content);
  for (const page of pages.slice(1)) {
    const existing = new Set(editor.getPages().map((p) => p.id));
    editor.createPage({ name: page.name });
    const created = editor.getPages().find((p) => !existing.has(p.id));
    if (!created) continue;
