 ...
 * the retired readiness checklist (#97). Connection state and project status
...
import type { ProjectStatus } from "@ai-storm/shared";
...
export function sessionIndicator(conn: ConnectionState, attached: boolean, status: ProjectStatus): SessionIndicator {
  if (conn === "closed") {
    return {
      tone: "error",
