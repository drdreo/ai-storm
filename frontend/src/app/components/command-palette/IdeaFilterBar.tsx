...
export function IdeaFilterBar({
    ...
    return (
        <div className="flex flex-wrap items-center gap-1.5 px-2 py-2">
            ...
            <FilterChip active={filter.markedOnly}
                        onClick={() => onChange({ ...filter, markedOnly: !filter.markedOnly })}>
                <Star className="size-3"/> Marked
            </FilterChip>
            <FilterChip active={filter.triagedOnly}
                        onClick={() => onChange({ ...filter, triagedOnly: !filter.triagedOnly })}>
